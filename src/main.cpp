...
#if defined(NDEBUG)
# error "Bitfilcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x937c2c816305b15d5f0a55477e3888cf4c7ae30a13aaedcba089804abff7a886");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitfilcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitfilcoin Signed Message:\n";
...
{
    // Bitfilcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 8300000000 * COIN;
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bitfilcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Bitfilcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitfilcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitfilcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc20db40b125102c4bd1cd717ca4717abf1cedf95a4da7fc2e2e6133810bb86ba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp ="Pakistan came into being 1947";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4150000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046150a3cf5e02f644686104e8c7392aa8a58ad0047b21d8935de6d06c506c26cea0d8f19e17b46c638b583c62dd70c725e7380dff30d84bdae8ff25d2786ffa51") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526335548;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085393477;

        if (fTestNet)
        {
            block.nTime    = 1526335514;
            block.nNonce   = 386055748;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x26e704299e77daf0f38320bbfcc5255430655c6ddcdfa8ce1343b0e87c054444"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xcb, 0xb3, 0xd2 }; // Bitfilcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitfilcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitfilcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitfilcoinMiner(CWallet *pwallet)
{
    printf("BitfilcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitfilcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitfilcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitfilcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitfilcoinMiner, pwallet));
}
